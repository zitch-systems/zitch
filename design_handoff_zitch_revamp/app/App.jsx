...
(function () {
  ...
  function App() {
    ...
    const [stack, setStack] = useState([{ key: 'splash' }]);
    ...
    useEffect(() => {
      if (mode !== 'app' || pasteRef.current) return;
    ...
    useEffect(() => {
      const f = () => { const sh = (window.innerHeight - 88) / dim.h; const sw = (window.innerWidth - 28) / dim.w; setScale(Math.min(device === 'phone' ? 1.35 : 1, sh, sw)); };
    ...
    const app = { theme, setTheme, balance, txns, showBal, setShowBal, biometrics, setBiometrics, beneficiaries, addBeneficiary, pay, fund, addTxn, toast: showToast, detected, clearDetected: () => setDetected(null), device, wide, tab, mode, enterApp: () => { try { localStorage.setItem('z-onboarded', '1'); } catch (e) { } setMode('app'); setTab('home'); setStack([]); }, nav };
    ...
    return (
      React.createElement(AppCtx.Provider, { value: app },
        React.createElement('div', { style: { position: 'fixed', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(120% 90% at 50% 0%, #16302B, #0A1614 70%)', overflow: 'hidden' } },
          // top control bar
          React.createElement('div', { style: { position: 'absolute', top: 0, left: 0, right: 0, height: 54, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 18px', color: 'rgba(255,255,255,.8)', zIndex: 100 } },
          ...
          React.createElement('div', { style: { zoom: scale } },
            React.createElement('div', { style: { width: dim.w, height: dim.h, borderRadius: dim.r, background: '#05100E', padding: 12, boxShadow: '0 40px 90px -30px rgba(0,0,0,.8), inset 0 0 0 2px #1d2a27' } },
              React.createElement('div', { className: 'z-' + theme, style: { position: 'relative', width: '100%', height: '100%', borderRadius: dim.r - 12, overflow: 'hidden', background: 'var(--bg)', display: 'flex' } },
