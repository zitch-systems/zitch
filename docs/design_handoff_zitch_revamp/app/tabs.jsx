...
(function () {
  ...
  function TxnRow({ x, divider, onClick }) {
    const neg = x.amt < 0;
    return <Tap onClick={onClick}><div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderTop: divider ? '1px solid var(--line)' : 'none' }}>
      <Monogram text={x.mono} color={x.col} />
      <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontWeight: 600, fontSize: 14.5, color: 'var(--ink-1)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{x.t}</div><div style={{ fontSize: 12, color: 'var(--ink-3)', marginTop: 2 }}>{x.time || 'Just now'}</div></div>
      <div style={{ textAlign: 'right' }}><div className="z-num" style={{ fontWeight: 700, fontSize: 14.5, color: neg ? 'var(--ink-1)' : 'var(--z-lime)' }}>{(neg ? '-' : '+') + fmtN(Math.abs(x.amt))}</div><div style={{ fontSize: 11, color: x.status === 'Pending' ? 'var(--z-amber)' : 'var(--ink-3)', marginTop: 3 }}>{x.status || 'Successful'}</div></div>
    </div></Tap>;
  }
  ...
  function BottomNav() {
    const app = useApp();
    const items = [['home', 'Home'], ['wallet', 'Wallet'], ['loan', 'Loans'], ['card', 'Cards'], ['user', 'Me']];
    const map = { home: 'home', wallet: 'wallet', loan: 'loans', card: 'cards', user: 'me' };
    return (
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 30 }}>
        <div style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', padding: '10px 8px 6px', background: 'color-mix(in srgb, var(--surface) 90%, transparent)', backdropFilter: 'blur(18px)', borderTop: '1px solid var(--line)' }}>
          {items.map(([ic, lb]) => {
            const on = app.tab === map[ic];
            return <Tap key={ic} onClick={() => app.nav.tab(map[ic])}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, padding: '2px 14px', color: on ? 'var(--brand)' : 'var(--ink-3)' }}>
                <I name={ic} size={22} stroke={on ? 2.1 : 1.7} />
                <span style={{ fontSize: 11, fontWeight: on ? 600 : 500 }}>{lb}</span>
              </div>
            </Tap>;
          })}
        </div>
      ...
      <TabRoot>
        ...
        <div style={{ margin: '14px 16px 0', borderRadius: 22, background: 'var(--hero-grad)', padding: '16px 18px 18px', position: 'relative', overflow: 'hidden', boxShadow: '0 16px 36px -18px rgba(0,132,123,.75)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 12 }}>
            <Tap onClick={() => { try { navigator.clipboard.writeText('9012345678'); } catch (e) { } app.toast('Account number copied'); }}><div style={{ display: 'flex', alignItems: 'center', gap: 7, color: 'rgba(255,255,255,.82)', fontSize: 12.5 }}><span className="z-num" style={{ fontWeight: 500 }}>Acct: 9012 345 678</span><I name="copy" size={14} color="rgba(255,255,255,.82)" /></div></Tap>
        ...
        <Tap onClick={() => n.push('coming', { title: 'Daily Interest', icon: 'spark', note: 'Earn interest on your balance, paid daily.' })}>
          <div style={{ margin: '12px 16px 0', borderRadius: 16, padding: '11px 14px', background: 'var(--surface)', boxShadow: 'var(--shadow-card)', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <Tap key={s.key} onClick={() => s.go(n, setMore)}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7 }}>
                  <div style={{ position: 'relative' }}>
                    <div style={{ width: 48, height: 48, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', background: app.theme === 'dark' ? 'rgba(15,162,149,.18)' : 'var(--z-teal-50)' }}><I name={s.icon} size={23} color="var(--brand)" /></div>
  ...
  function Wallet() {
    ...
    return <TabRoot>
      <div style={{ padding: '6px 20px 2px', fontSize: 26, fontWeight: 800, color: 'var(--ink-1)' }}>Wallet</div>
      <div style={{ margin: '14px 16px 0', borderRadius: 22, background: 'var(--hero-grad)', color: '#fff', padding: 20, position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', right: -20, bottom: -26, opacity: .16 }}><ZMark size={140} /></div>
        <div style={{ fontSize: 13, opacity: .85 }}>Total balance</div>
        <div className="z-num" style={{ fontSize: 32, fontWeight: 800, marginTop: 4 }}>{app.showBal ? fmtN(app.balance) : '₦ ••••••'}</div>
        <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
          <Tap onClick={() => n.push('addmoney')} style={{ flex: 1 }}><div style={{ textAlign: 'center', padding: '12px', borderRadius: 13, background: '#fff', color: 'var(--brand-deep)', fontWeight: 700, fontSize: 14 }}>+ Add money</div></Tap>
          <Tap onClick={() => n.push('transfer')} style={{ flex: 1 }}><div style={{ textAlign: 'center', padding: '12px', borderRadius: 13, background: 'rgba(255,255,255,.18)', color: '#fff', fontWeight: 700, fontSize: 14, border: '1px solid rgba(255,255,255,.25)' }}>Send</div></Tap>
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 12, margin: '14px 16px 0' }}>
        {[['Money in', '+₦150,000', 'var(--z-lime)'], ['Money out', '-₦94,100', 'var(--ink-1)']].map(([k, v, c]) => (
          <div key={k} style={{ borderRadius: 16, background: 'var(--surface)', boxShadow: 'var(--shadow-card)', padding: 16 }}>
            <div style={{ fontSize: 12.5, color: 'var(--ink-3)' }}>{k} · May</div>
            <div className="z-num" style={{ fontSize: 18, fontWeight: 800, color: c, marginTop: 4 }}>{v}</div>
          </div>
        ))}
      </div>
      <div style={{ padding: '20px 18px 0' }}>
        <SectionLabel action="Filter">Transactions</SectionLabel>
        {app.txns.map((x, i) => <TxnRow key={x.id || i} x={x} divider={i > 0} onClick={() => n.push('txn', { x })} />)}
      </div>
    </TabRoot>;
  }
  ...
  function Cards() {
    ...
    return <TabRoot>
      ...
      <div style={{ display: 'flex', gap: 10, margin: '16px 16px 0' }}>
        {[['lock', freeze ? 'Unfreeze' : 'Freeze', () => setFreeze(f => !f)], ['eye', 'Details', () => n.push('coming', { title: 'Card details', icon: 'card', note: 'View full card number & CVV.' })], ['settings', 'Settings', () => n.push('coming', { title: 'Card settings', icon: 'settings', note: 'Limits, channels & more.' })]].map(([ic, lb, go]) => (
          <Tap key={lb} onClick={go} style={{ flex: 1 }}><div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '14px 6px', borderRadius: 16, background: 'var(--surface)', boxShadow: 'var(--shadow-card)' }}><I name={ic} size={20} color="var(--brand)" /><span style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink-2)' }}>{lb}</span></div></Tap>
        ))}
      </div>
