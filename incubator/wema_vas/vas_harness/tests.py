...
class ContractTests(APIHelpers, TestCase):
    ...
    def test_bank_ack_discrepancy_does_not_repost_money(self):
        self.post("transaction-notification", payload())
        result = reconcile_snapshot(self.snapshot(sendresponse="07"))[0]
        self.assertEqual(result["local_comparison"], "bank_ack_discrepancy_manual_review")
        self.assertEqual(VirtualAccount.objects.get(number=NUMBER).simulated_balance, Decimal("3100.05"))

    def test_read_only_audit_detects_balance_tampering(self):
        self.post("transaction-notification", payload())
        VirtualAccount.objects.filter(number=NUMBER).update(simulated_balance=Decimal("1.00"))
        output = io.StringIO()
        call_command("audit_inflows", stdout=output)
        findings = json.loads(output.getvalue())["findings"]
        self.assertEqual(findings[0]["action"], "manual_bank_evidence_review")
        self.assertEqual(findings[0]["account"], NUMBER)
        self.assertEqual(VirtualAccount.objects.get(number=NUMBER).simulated_balance, Decimal("1.00"))
...
class IsolationTests(SimpleTestCase):
    ...
    def test_validation_mode_refuses_missing_keys_or_database(self):
        with patch.dict(os.environ, {"WEMA_VAS_MODE": "validation"}):
            spec = importlib.util.find_spec("vas_harness.settings")
            module = importlib.util.module_from_spec(spec)
            with self.assertRaises(ImproperlyConfigured):
                spec.loader.exec_module(module)
...
@override_settings(VAS_ENABLED=True, VAS_MODE="validation", VAS_TOKEN=VALIDATION_TOKEN,
                   VAS_PREFIX="711", VAS_IDENTITY_KEYS=KEY, VAS_REQUIRE_HTTPS=True)
class ValidationContractTests(TestCase):
    @classmethod
    def setUpTestData(cls):
        VirtualAccount.objects.create(
            number=VALIDATION_NUMBER, mode="validation", customer_reference="TEST-ONLY-001",
            display_name="Zitch/VALIDATION SAMPLE", encrypted_identity=encrypt_identity(
    ...
    def post(self, route, data):
        return self.client.post("/vas/" + route, data, content_type="application/json",
                                HTTP_AUTHORIZATION="Bearer " + VALIDATION_TOKEN, secure=True)

    def test_requires_https_even_with_valid_token(self):
        result = self.client.post("/vas/account-lookup", {"accountnumber": VALIDATION_NUMBER},
                                  content_type="application/json", HTTP_AUTHORIZATION="Bearer " + VALIDATION_TOKEN)
        self.assertEqual(result.status_code, 403)

    def test_identity_is_encrypted_and_available_to_bank_endpoints(self):
        account = VirtualAccount.objects.get(number=VALIDATION_NUMBER)
        self.assertNotIn("11111111111", account.encrypted_identity)
        self.assertNotIn("08000000000", account.encrypted_identity)
        self.assertEqual(self.post("account-lookup", {"accountnumber": VALIDATION_NUMBER}).json(), {
            "accountname": "Zitch/VALIDATION SAMPLE", "bvn": "11111111111", "nin": "",
            "status": "00", "status_desc": "Okay"})
        self.assertEqual(self.post("kyc-details", {"accountnumber": VALIDATION_NUMBER}).json(), {
            "accountname": "Zitch/VALIDATION SAMPLE", "bvn": "11111111111", "nin": "",
            "mobilenumber": "08000000000", "walletbalance": "0.00", "status_desc": "Active"})

    def test_synthetic_records_are_never_visible_in_validation_mode(self):
        self.assertEqual(self.post("account-lookup", {"accountnumber": NUMBER}).json()["status"], "07")
        with self.assertRaises(ValidationError):
            VirtualAccount.objects.create(number=NUMBER, mode="synthetic")

    def test_notification_to_balance_statement_block_and_replay(self):
        data = payload(craccount=VALIDATION_NUMBER, craccountname="Zitch/VALIDATION SAMPLE")
        first = self.post("transaction-notification", data)
        self.assertEqual(first.json()["status"], "00")
        self.assertEqual(self.post("transaction-notification", data).json(), first.json())
        self.assertEqual(self.post("kyc-details", {"accountnumber": VALIDATION_NUMBER}).json()["walletbalance"], "3100.05")
        self.assertEqual(len(self.post("mini-statement", {"accountnumber": VALIDATION_NUMBER}).json()["transactions"]), 1)
        self.assertEqual(self.post("block-account", {"accountnumber": VALIDATION_NUMBER, "blockreason": "Review"}).status_code, 200)
        self.assertEqual(self.post("account-lookup", {"accountnumber": VALIDATION_NUMBER}).json()["status_desc"], "Inactive Account")
        self.assertEqual(self.post("transaction-notification", data).json(), first.json())
        self.assertEqual(self.post("transaction-notification", payload(craccount=VALIDATION_NUMBER,
            craccountname="Zitch/VALIDATION SAMPLE", sessionid="SIM-NEW", paymentreference="SIM-NEW-P")).json()["status"], "07")
        self.assertEqual(self.post("kyc-details", {"accountnumber": VALIDATION_NUMBER}).json()["walletbalance"], "3100.05")

    def test_missing_decryption_key_fails_closed(self):
        with override_settings(VAS_IDENTITY_KEYS=OTHER_KEY):
            self.assertEqual(self.post("kyc-details", {"accountnumber": VALIDATION_NUMBER}).status_code, 503)
            self.assertEqual(self.post("transaction-notification", payload(craccount=VALIDATION_NUMBER,
                craccountname="Zitch/VALIDATION SAMPLE")).status_code, 503)
        self.assertEqual(Inflow.objects.count(), 0)

    def test_multi_key_rotation_reads_previous_identity(self):
        with override_settings(VAS_IDENTITY_KEYS=OTHER_KEY + "," + KEY):
            self.assertEqual(decrypt_identity(VirtualAccount.objects.get().encrypted_identity)["bvn"], "11111111111")
            fresh = encrypt_identity(bvn="", nin="22222222222", phone="08000000000")
        with override_settings(VAS_IDENTITY_KEYS=OTHER_KEY):
            self.assertEqual(decrypt_identity(fresh)["nin"], "22222222222")

    def test_rotation_command_reencrypts_without_printing_identity(self):
        previous = VirtualAccount.objects.get().encrypted_identity
        output = io.StringIO()
        with override_settings(VAS_IDENTITY_KEYS=OTHER_KEY + "," + KEY):
            call_command("rotate_identities", stdout=output)
        latest = VirtualAccount.objects.get().encrypted_identity
        self.assertNotEqual(previous, latest)
        self.assertNotIn("11111111111", output.getvalue())
        with override_settings(VAS_IDENTITY_KEYS=OTHER_KEY):
            self.assertEqual(decrypt_identity(latest)["bvn"], "11111111111")
        with override_settings(VAS_IDENTITY_KEYS=KEY):
            with self.assertRaises(ImproperlyConfigured):
                decrypt_identity(latest)

    def test_validation_account_requires_evidence_and_valid_prefix(self):
        for number in ("7110000001", "1233000001"):
            with self.assertRaises(ValidationError):
                VirtualAccount.objects.create(number=number, mode="validation", customer_reference="other",
                    display_name="Zitch/TEST", encrypted_identity="encrypted", verified_at=timezone.now())

    def test_handoff_needs_three_approved_accounts_and_never_exports_token(self):
        with override_settings(ALLOWED_HOSTS=["testserver", "vas.example.test"]):
            with self.assertRaisesMessage(Exception, "Three approved"):
                call_command("bank_handoff", base_url="https://vas.example.test")
            existing = VirtualAccount.objects.get()
            for suffix in ("2", "3"):
                VirtualAccount.objects.create(
                    number="711300000" + suffix, mode="validation",
                    customer_reference="TEST-ONLY-00" + suffix, display_name=existing.display_name,
        ...
        self.assertEqual(len(checklist["sample_accounts"]), 3)
        self.assertEqual(len(checklist["endpoints"]), 5)
        self.assertNotIn(VALIDATION_TOKEN, output.getvalue())
        self.assertNotIn("11111111111", output.getvalue())
