...
OUR OWN float at the VAS provider, in a sentence the provider writes in the second
person. The customer was told by their bank that their money had gone.
...
The VAS rail has since moved to the partner bank, and the rule is unchanged — the
argument above is about the ORDER of the debit and the provider call, not about which
provider. Any rail can answer "insufficient balance" about its own float in the second
person, so the wording below stays as the fixture: it is the real sentence that reached
a real customer, and it is the shape the regex has to catch.
"""
...
# The provider's exact wording, from the screenshots.
PROVIDER_FLOAT_EMPTY = ("Your wallet balance (NGN12.25) is insufficient to make "
...
class CustomerSafeFailureTests(TestCase):
    def test_the_reported_message_is_never_shown_to_the_customer(self):
        said = customer_safe_failure({"message": PROVIDER_FLOAT_EMPTY}, service="airtime")
    ...
    def test_an_exhausted_float_pages_an_operator(self):
        ...
        with self.assertLogs("zitch", level="ERROR") as logs:
            customer_safe_failure({"message": PROVIDER_FLOAT_EMPTY}, service="airtime")
        joined = "\n".join(logs.output)
...
class TheCustomersMoneyIsUntouchedTests(TestCase):
    ...
    def test_a_failed_purchase_leaves_the_balance_where_it_was(self):
        ...
        status, txn, result = run_provider_purchase(
            ...
            lambda ref: {"success": False, "message": PROVIDER_FLOAT_EMPTY},
        )
