...
#
# Airtime and Data are covered by our Wallet Services subscription and are listed
# here on that basis — confirmed against the tenant's own subscription, which is why
# purchases authenticate without a separate WEMA_AIRTIME_KEY. `purchase_data` posts
# under the same "airtime" product, so the one entry covers both.
#
# Worth recording what this list is NOT responsible for, because two different
# failures were blamed on it during the September airtime outage and neither was an
# entitlement problem:
#
#   * "Authentication Failed" on a purchase came from OUR side. ALAT calls our
#     authentication callback to ask whether a transaction may proceed, and
#     `_authorize_payout` was admitting bank payouts only — so every airtime purchase
#     was answered no and the bank relayed the denial to the customer. See
#     wallet/wema_callbacks.py.
#   * "You've not been profiled to use this service" came from the PartnerPayment
#     STATUS endpoint, which is a different ALAT product from the purchase endpoint
#     and is entitled separately. Selling can work while settling does not; see
#     vas_status_entitlement.
#
# The list stays deliberately narrower than what a tenant's subscription may APPEAR
# to allow. An API appearing in a product's documented list is not the same as the
# tenant being subscribed to it, so a product goes in here once the gateway has been
# seen to accept the wallet key for it, and `manage.py wema_preflight` asks the
# gateway rather than trusting the list. Explicit beats inferred, and it stays
# visible when products are later split onto their own subscriptions.
#
...
_WALLET_COVERED = ("wallet_nin", "wallet_bvn", "acct_mgt", "credit", "debit",
                   "airtime", "bills")
...
def _sub_key(product: str) -> str:
    ...
    Wallet Services covers the rails listed in _WALLET_COVERED. Every other product is
...
def face_verify_live() -> bool:
    ...
    if not (_face_key() and settings.WEMA.get("FACE_VERIFY_URL")):
        return False
    # Server-to-server callbacks require the verifier IP allowlist. Browser-delivered
    # callbacks are admitted only from the exact configured HTTPS origin and are not
    # trusted on origin alone: wema_face_callback proves the returned correlation by
    # submitting it to Wema's authenticated without-OTP endpoint before changing KYC.
    if getattr(settings, "DEBUG", False) or getattr(settings, "TESTING", False):
        return True
    origins = settings.WEMA.get("FACE_CALLBACK_ORIGINS") or ()
    has_https_origin = any(str(origin).startswith("https://") for origin in origins)
    return bool(settings.WEMA.get("FACE_CALLBACK_IPS") or has_https_origin)
