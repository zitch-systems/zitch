"""Monnify integration â€” the single money-movement rail for Zitch.
...
checkout page, and â€” via the Disbursement API â€” sends money out and does recipient
...
whitelisting, but Monnify *disbursements* (payouts) DO â€” the calling server's
egress IP must be whitelisted in the Monnify dashboard (Settings > API), OR 2FA/OTP
must be authorized per transfer. Zitch uses the IP-whitelist model. Because the app
host (Render) has its own outbound IP, ALL Monnify calls are routed through an HTTPS
forward proxy (``MONNIFY_PROXY_URL``, e.g. a DigitalOcean box with a static IP) so
Monnify sees that one whitelistable IP. Blank => call Monnify directly. Use
``egress_ip()`` / ``disbursement_diagnostics()`` to confirm the IP Monnify sees
(measured THROUGH the proxy) matches the whitelisted one.

Auth: OAuth login â€” Basic base64(apiKey:secretKey) -> bearer access token (~1h),
...
is testable offline â€” EXCEPT in production (DEBUG off), where account/money calls
...
production (to demo/test a real build without live keys) â€” no real money moves.
...
be exercised from CI â€” confirm against the dashboard before go-live.
"""
import base64
import hashlib
import hmac
...
def monnify_simulation() -> bool:
    """Whether MONNIFY_SIMULATION is on â€” serve the mock fund-in flow even in
...
def _proxies() -> dict | None:
    """Route Monnify egress through MONNIFY_PROXY_URL when set, so Monnify sees one
    whitelistable static IP (e.g. a DigitalOcean box) instead of the host's own
    outbound IP. Applied to EVERY Monnify call. Blank => call Monnify directly,
    unchanged. Required for disbursements when the whitelisted IP isn't the app's
    own egress."""
...
def _monnify_token() -> str:
    ...
    until shortly before it expires. Returns "" on failure â€” callers guard on an
...
# ---------------------------------------------------------------------------
# Hosted checkout (card / bank) â€” fund the wallet without a dedicated account
# ---------------------------------------------------------------------------
def payment_initialize(email: str, amount_naira, reference: str, *,
    ...
    if headers is None:
        return {"success": False, "message": (
            "Payment gateway authentication failed â€” verify MONNIFY_API_KEY/SECRET_KEY and that "
...
def payment_verify(reference: str) -> dict:
    ...
    try:
        resp = requests.get(
            ...
            params={"paymentReference": reference}, headers=headers, timeout=REQUEST_TIMEOUT, proxies=_proxies(),
        )
        data = resp.json()
        rb = data.get("responseBody", {}) or {}
...
# ---------------------------------------------------------------------------
# Reserved (dedicated) virtual accounts â€” fund by bank transfer
# ---------------------------------------------------------------------------
...
def create_virtual_account(account_reference: str, account_name: str, customer_email: str,
    ...
    stable per-user key (Monnify rejects a duplicate â€” reuse it). Monnify requires
...
# ---------------------------------------------------------------------------
# Payouts / disbursements â€” the "sender" rail (Monnify Disbursement API)
#
...
# closed in production when unkeyed (mock_disabled_in_prod) â€” a simulated payout is
...
def disburse(amount_naira, reference: str, narration: str, bank_code: str,
    ...
    if not src:
        ...
        return {"success": False,
                "message": "Payouts are temporarily unavailable â€” please try again shortly."}
    m = settings.MONNIFY
    body = {
        "amount": float(amount_naira),  # Monnify amounts are in naira
        "reference": reference,
...
def verify_payout(reference: str) -> dict:
    ...
    try:
        resp = requests.get(
            ...
            params={"reference": reference}, headers=headers, timeout=REQUEST_TIMEOUT, proxies=_proxies(),
        )
        data = resp.json()
        rb = data.get("responseBody", {}) or {}
        status = str(rb.get("status", "")).lower()
...
def get_wallet_balance(account_number: str = "") -> dict:
    ...
    try:
        resp = requests.get(
            ...
            params={"accountNumber": src}, headers=headers, timeout=REQUEST_TIMEOUT, proxies=_proxies(),
        )
        data = resp.json()
        rb = data.get("responseBody", {}) or {}
...
def egress_ip() -> dict:
    """Report the server's public egress IP â€” the address Monnify sees on a
    ...
    try:
        resp = requests.get("https://api.ipify.org", params={"format": "json"}, timeout=10, proxies=_proxies())
...
# ---------------------------------------------------------------------------
# KYC â€” BVN details-match + NIN lookup (Monnify VAS)
#
...
# product â€” it is verified via Prembly (see providers.kyc_verify_vnin). Mock when
...
def verify_bvn(bvn: str, name: str = "", date_of_birth: str = "", mobile: str = "") -> dict:
    ...
    Monnify doesn't return the holder's data â€” it MATCHES what we supply, so pass
    ...
    if not monnify_live():
        # Identity NEVER mock-passes in production â€” deliberately stricter than
...
def verify_nin(nin: str) -> dict:
    ...
    try:
        resp = requests.post(f"{m['BASE_URL']}/api/v1/vas/nin-details",
                             json={"nin": nin}, headers=headers, timeout=REQUEST_TIMEOUT, proxies=_proxies())
        data = resp.json()
...
# ---------------------------------------------------------------------------
# Diagnostics â€” mirrors mono_diagnostics
# ---------------------------------------------------------------------------
def deallocate_virtual_account(account_reference: str) -> dict:
    """Deallocate (release) a reserved account by our accountReference â€”
...
def monnify_probe(bvn: str = "", name: str = "", email: str = "") -> dict:
    ...
    product is accessible, and â€” optionally, when ?bvn=&name= are supplied â€”
    ...
    out = {"config": monnify_diagnostics(),
           "webhook_reminder": "Funding credits arrive via the webhook â€” set "
                               "https://<your-api-host>/api/fund/monnify/webhook/ in the "
                               "Monnify dashboard (Settings > Webhooks) or deposits will NOT credit."}
    if not monnify_live():
        out["hint"] = "Monnify keys incomplete â€” no live call was made."
        return out
    ...
    if not token_ok:
        out["auth"]["hint"] = ("Login failed â€” check MONNIFY_API_KEY/MONNIFY_SECRET_KEY and that "
    ...
    if bvn and name:
        ...
        out["nuban_create"] = {"ok": created.get("success"),
                               ...
                               "note": "Real reserved account created for diagnosis â€” deallocate "
...
def monnify_diagnostics() -> dict:
    ...
    if not monnify_live():
        if monnify_simulation():
            ...
            out["hint"] = ("MONNIFY_SIMULATION is ON â€” the mock fund-in flow is served even in "
...
def disbursement_diagnostics() -> dict:
    """Structured Monnify PAYOUT self-test (no secrets) â€” the sender-side probe.
    ...
    m = settings.MONNIFY
    out = {
        "egress": egress_ip(),  # the IP Monnify sees â€” must be whitelisted
    ...
    if not out["auth_ok"]:
        ...
        out["hint"] = ("Wallet-balance read failed â€” either the keys are wrong, or (most likely for a "
    ...
    out["status"] = "ok"
    out["hint"] = ("Auth + wallet read work and the egress IP authenticates â€” the IP-whitelist payout "
