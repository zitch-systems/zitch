...
Providers: Monnify (payments), VTU.ng / ClubConnect / Baxi (VTU — airtime, data,
cable, electricity, betting, exam e-PIN; selected by settings.VTU_PROVIDER and
dispatched via vtu_purchase / vtu_requery / vtu_verify_customer), Sendchamp
(SMS/OTP), Prembly/IdentityPass (KYC: BVN/NIN/face). Each function returns
{"success": bool, ...}. When the relevant key is blank it runs in MOCK mode and
simulates success so the whole app flow is testable without an external account —
EXCEPT in production (DEBUG off), where the VTU mock fails closed (see
mock_disabled_in_prod) so a misconfigured deploy never fakes a money movement.
