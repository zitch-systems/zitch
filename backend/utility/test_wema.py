...
class WemaRenderBlueprintTests(SimpleTestCase):
    def test_wema_crons_share_the_web_simulation_safety_switches(self):
        ...
        # These are the crons that TALK TO A RAIL, and so must know whether the
        # deploy is simulated — otherwise a cron makes real calls while the web
        # service believes nothing can move money. zitch-integrity-check is absent
        # on purpose: it compares the ledger to our own stored balances and reaches
        # no provider at all. zitch-reconcile-vtu is absent because it no longer
        # exists — VAS settlement moved into reconcile_wema when the retired rail was
        # removed, and the cron was deleted with it.
        for service in (
